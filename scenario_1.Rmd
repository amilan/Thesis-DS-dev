...
```{r}
# Instead of here we can also apply this normalization just before merging the datasets.
...
# Indexes for the genes to keep as
...
#sort(matched_idx)
#matched_idx
#pData(featureData(ranked_gse_21[2]))$ENTREZ_GENE_ID
#pData(featureData(ranked_gse_25[23]))$Entrez_Gene_ID
#pData(featureData(ranked_gse_25[matched_idx]))$Entrez_Gene_ID

# Remove the entries that doesn't appear in the ranked_gse_21
#which(apply(exprs(ranked_gse_25[matched_idx]), 1, function(x) any ( is.na(x))))
#idx_to_rmv <- which(apply(exprs(ranked_gse_25[matched_idx]), 1, function(x) any ( is.na(x))))
#idx_to_rmv
#common_gse_25 <- ranked_gse_25[-idx_to_rmv,]
#common_gse_25 <- ranked_gse_25[sort(matched_idx)]
...
length(matched_idx_2)
matched_idx_2
common_gse_21 <- common_gse_21[matched_idx_2]
##remove NA
#idx_to_rmv <- which(apply(pData(featureData(common_gse_21)), 1, function(x) any ( is.na(x))))
#common_gse_21 <- common_gse_21[-idx_to_rmv,]
```

```{r}
# Get the common genes from gse_21
#matched_idx
#idx_to_keep <- is.na(matched_idx)
#!idx_to_keep
#common_gse_21 <- ranked_gse_21[!idx_to_keep]
...
```{r}
# Merge both datasets in one by adding columns
